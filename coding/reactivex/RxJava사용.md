...
- Subject(Observable) : 이벤트(데이터 조회/변환하는 mechanism)를 발생시킨다.
- Observer(Subscriber) : 발생한 이벤트를 받아 처리한다. 이벤트의 발생을 감지한다.
...
- observe : 구독보다는 좀 더 넓은 범위의 행위, 관찰을 한다고 해서 구독을 하는 것은 아니다.
...
OnSubscribe 는 Observable.subscribe가 호출 될 때 호출됩니다.
```java
...
[Rxjava와 RxAndroid 소개](http://freemmer.tistory.com/126)
